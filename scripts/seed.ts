...
const TEST_AUTH_IDS = {
  user1: "3a9eaef9-367c-40a1-9141-ba5d0f8b9068", // test@youos.app
  user2: "788025a7-b708-44ef-8d5c-1d609ebd77ed", // sarah@youos.app
  user3: "7095f3dd-5c51-4baf-a1c0-b37ce1b7edd5", // owner@acme.com
  user4: "3883ea94-37cb-44b7-b1d7-9036b7031150", // candidate@acme.com
};
        ...
        analysis:
          i <= 3
            ? {
                faceDetected: true,
                faceCount: 1,
                facialExpression: ["smiling", "neutral", "confident"][
                ...
                background: ["outdoor", "indoor", "urban"][
                  Math.floor(Math.random() * 3)
                ],
                styleCategory: ["casual", "professional", "creative"],
                colorDominant: ["blue", "neutral", "warm"],
                mood: ["friendly", "confident", "approachable"][
                  Math.floor(Math.random() * 3)
                ],
                issues:
                  i === 3 ? ["slight blur", "background distraction"] : [],
                suggestions:
                  i === 3
...
Let's connect if you're building something meaningful.`,
    prompt:
      "Generate a LinkedIn summary that showcases technical expertise while remaining approachable",
...
async function seedSignupInviteTokens(admins: schema.AdminUser[]) {
  ...
  const invites = await db
    .insert(schema.signupInviteTokens)
    .values(invitesData)
    .returning();
...
async function main() {
  ...
  try {
    ...
    console.log(
      "   Token: VIP-INVITE-2025 (1 use, restricted to vip@example.com)"
    );
    console.log(
      "\n⚠️  Remember to create these users in Supabase Auth with matching auth IDs!"
    );
    console.log(
      "⚠️  Set INVITE_ONLY_REGISTRATION=true to enable invite-only mode."
    );
  } catch (error) {
    console.error("\n❌ Seed failed:", error);
    process.exit(1);
